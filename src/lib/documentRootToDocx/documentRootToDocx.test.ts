...
import { documentRootToDocx } from './documentRootToDocx';
...
it('runs without error', async () => {
  ...
  const docx = documentRootToDocx(documentRoot);
  const buffer = await Packer.toBuffer(docx);
  await writeTestFile('documentRootToDocx.docx', buffer);
        ...
        sections: [
          {
            ...
            headers: {
              first: new D.Header({
                children: [new D.Paragraph({ text: 'first' })],
              }),
              even: new D.Header({
                children: [new D.Paragraph({ text: 'even' })],
              }),
              default: new D.Header({
                children: [new D.Paragraph({ text: 'odd' })],
              }),
            },
...
function mockContent() {
  ...
  return [...arr, ...arr, ...arr].map((t) => new D.Paragraph({ text: t }));
}

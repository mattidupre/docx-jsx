...
export type PageVars = Partial<
  Record<keyof (typeof Pager)['PAGE_OPTIONS_DEFAULTS'], string>
>;
...
let baseStyleSheets: undefined | ReadonlyArray<CSSStyleSheet>;

export class Pager {
  // Memoize CSSStyleSheet instantiation.
  static get baseStyleSheets() {
    if (!baseStyleSheets) {
      const baseStyleSheet = new CSSStyleSheet();
      baseStyleSheet.replaceSync(Pager.BASE_STYLE);

      const varsStyleSheet = Pager.varsToStyleSheet(
        Pager.PAGE_OPTIONS_DEFAULTS,
      )!;

      baseStyleSheets = [baseStyleSheet, varsStyleSheet];
    }
  ...
  static parseVars(
    { width, height, ...vars }: PageVars,
    { preservePageTypes }: PageVarsOptions = {},
  ): Array<[string, string]> {
    const widthVars =
      !preservePageTypes && width
        ? {
            width,
            pageboxWidth: width,
            widthRight: width,
    ...
    return Object.entries({
      ...
      `--pagedjs-${key.replace(
        /[A-Z]+(?![a-z])|[A-Z]/g,
        (match: string, nextMatch: string) =>
          (nextMatch ? '-' : '') + match.toLowerCase(),
      )}`,
      value,
    ]);
  }

  static varsToStyleSheet(vars?: PageVars, options: PageVarsOptions = {}) {
    if (!vars) {
      return undefined;
    }
    const parsedVars = this.parseVars(vars, options);
    if (!parsedVars.length) {
      return undefined;
    }
    const styleSheet = new CSSStyleSheet();

    // TODO: What does ROOT do inside shadow DOM?
    styleSheet.insertRule(`:host, :root {
      ${parsedVars.map(([key, value]) => `${key}: ${value};`).join(' ')}
    }`);
    return styleSheet;
  }

  static applyVarsToElement(
    vars: undefined | PageVars,
    element: HTMLElement,
    options: PageVarsOptions = {},
  ) {
    if (!vars) {
      return;
    }
  ...
  static readonly PAGE_OPTIONS_DEFAULTS = {
    width: '8.5in',
    height: '11in',
    widthRight: '8.5in',
    heightRight: '11in',
    ...
    cropStroke: '1px',
    crossSize: '5mm',
    markCrossDisplay: 'none',
    markCropDisplay: 'none',
    pageCount: '0',
    pageCounterIncrement: '1',
    footnotesCount: '0',
    columnGapOffset: '1000px',
